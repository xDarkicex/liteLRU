...
type LRUCache struct {
	...
	// HashMap maps a route hash to an SoA index
...
func (c *LRUCache) Add(method, path string, handler HandlerFunc, params []Param) {
	...
	c.mapMutex.Lock()
	c.hashIndex[hash] = uint32(victimIdx)
	c.mapMutex.Unlock()
	...
	if (validBits & (1 << bit)) != 0 {
		...
		c.mapMutex.Lock()
		if oldSlot, found := c.hashIndex[oldHash]; found && oldSlot == uint32(victimIdx) {
			delete(c.hashIndex, oldHash)
		}
		c.mapMutex.Unlock()
	}
...
func (c *LRUCache) Clear() {
	c.mapMutex.Lock()
	c.hashIndex = make(map[uint64]uint32)
	c.mapMutex.Unlock()
