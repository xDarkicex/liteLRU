...
\begin{enumerate}
    \item \textbf{Off-Heap HashMap:} \texttt{liteLRU} delegates key-value storage to a custom open-addressed hash map (\texttt{memory.HashMap}\footnote{\url{https://github.com/xDarkicex/memory}}) backed by an mmap-allocated region. This hash map implements SSE2/NEON SIMD-accelerated linear probing (\texttt{\_mm\_cmpeq\_epi8} / \texttt{vceqq\_u8}) on a 16-byte metadata array, strictly bounded to a maximum of 32 bucket probes (512 total items). This hard bound guarantees that map lookups operate in bounded time. To handle tombstone accumulation without blocking the hot path, the map utilizes a fully concurrent background Proportional-Integral-Derivative (PID) controller (triggered when average probe length exceeds a setpoint threshold) that incrementally rehashes elements when probe lengths exceed setpoint tolerances. As a result, the eviction algorithm only manages the \textit{indexes} of the hash map slots, and does not itself allocate or free memory.
...
\textbf{\texttt{liteLRU}}     & \textbf{31,140,774} & 583 ns      & \textbf{1.79 \textmu s} \\
\texttt{otter} v2             & 10,220,409          & \textbf{541 ns}      & 72.16 \textmu s \\
\texttt{Mutex LRU}            &  5,303,347          & 292 ns      & 41.29 \textmu s \\ \bottomrule
\end{tabular}
\caption{Baseline throughput and tail latency comparison.}
\end{table}

While the \texttt{Mutex LRU} achieves the lowest median latency (292ns) due to the absence of amortized background processing overhead, its p99 latency spikes significantly as goroutines convoy on the global lock. \texttt{otter} amortizes contention via background ingestion buffers, keeping median latency low (541ns) but pushing p99 to $>$70\textmu s when those buffers require synchronization.

\texttt{liteLRU} demonstrates a $>$3$\times$ throughput advantage over \texttt{otter} and a nearly 6$\times$ advantage over the mutex baseline. Crucially, its synchronous wait-free design tames the tail: the p99 latency (1.79\textmu s) is nearly 40$\times$ lower than \texttt{otter}'s.
...
\begin{itemize}
    ...
    \item \textbf{No Lock-Free Index}: Replaced the off-heap lock-free hash map with a standard Go \texttt{map} protected by a \texttt{sync.RWMutex}.
\end{itemize}
...
To evaluate eviction fidelity under realistic skewed access patterns, we benchmarked \texttt{liteLRU} against \texttt{otter} v2 using a Zipfian distribution ($s=1.001$, $N=100{,}000$ working set) with 1.6M total operations across varying cache capacities. Both caches were given a 20\% warmup phase.
...
\texttt{liteLRU}'s bitmask-CLOCK approximation achieves superior or tied hit rates against Otter's S3-FIFO-based eviction policy across all tested capacities. \texttt{liteLRU} achieves hit rates competitive with frequency-optimal caching across both high-skew and moderate-skew Zipfian distributions. Furthermore, CLOCK's recency tracking provides an advantage over pure frequency-based eviction at moderate skew (e.g. $s=0.8$) due to temporal locality in the access pattern, demonstrating that its structurally lock-free hit path trades no eviction fidelity for its speed. It should be noted that this hit-rate parity is characteristic of heavily-skewed ($s=1.001$) heavy-tailed distributions; on scan-resistant or low-skew workloads, S3-FIFO is expected to outperform CLOCK approximations. However, \texttt{liteLRU}'s throughput remains immune to the high hit-ratio contention pathology formally described by Qiu et al.~\cite{hitrate2026}.

\subsection{Write-Heavy Workloads}

We compare against Otter v2 rather than \texttt{ristretto} here because \texttt{ristretto}'s hit-rate collapses under intense concurrent pressure, rendering its write throughput an artifact of dropped samples rather than true admission. To stress the concurrent write protocol under severe contention, we measured throughput across a 1.6M operation Zipfian workload with aggressive Get/Add ratios using 8 concurrent cores.
...
50/50 Get/Add         & \textbf{18,127,975}               & 3,544,711                           & \textbf{5.11x}   \\
20/80 Get/Add         & \textbf{12,852,696}               & 1,460,176                           & \textbf{8.80x}   \\ \bottomrule
\end{tabular}
\caption{Throughput under write-heavy pressure.}
\end{table}

\texttt{liteLRU} demonstrates a massive 5.1x to 8.8x throughput advantage under write-heavy pressure. When writes dominate (80\%), amortized caches like \texttt{otter} experience severe contention on their ingestion buffers. \texttt{liteLRU} sustains nearly 13M ops/sec by confining state mutations to single-cycle hardware atomics (the $W_k$ bitmask).
