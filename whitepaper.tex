...
\end{tabular}
\caption{Baseline throughput and latency comparison against production caches. Latency percentiles were sampled at a 1\% rate to preserve realistic throughput levels.}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{No Mmap}: Replaced the off-heap lock-free hash map with a standard Go \texttt{map} protected by a \texttt{sync.RWMutex}.
\end{itemize}
...
No Mmap (Go Map + Mutex)         & 4,280,582  & -87\% \\ \bottomrule
\end{tabular}
