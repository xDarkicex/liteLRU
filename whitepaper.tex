...
\texttt{liteLRU} addresses this through two radical architectural mechanisms:

\begin{enumerate}
    \item \textbf{64-Way Set Associativity over Hash Maps:} Lock-free open-addressed hash maps mathematically require \textit{tombstones} during deletion to preserve concurrent probe sequences. When tombstones accumulate, lock-free maps are forced to perform global, cooperative compactions, resulting in massive, catastrophic tail latency spikes (often $>$100ms) under heavy eviction load. \texttt{liteLRU} completely eliminates the hash map. Instead, it utilizes a hardware-inspired \textbf{64-way set associative architecture}. An incoming route hash is mathematically mapped to a specific 64-slot bucket (a "set"). 
...
\subsection{Dynamic Router Integration (Path Params and Middleware)}
...
By caching the route resolution itself, \texttt{liteLRU} drops the median observed request latency in this benchmark from 381~$\mu$s to just 253~$\mu$s, proving that it acts as a tremendously effective layer for HTTP frameworks looking to bypass dynamic parameter extraction logic entirely.
