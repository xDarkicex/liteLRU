...
\texttt{liteLRU} (Bitmask CLOCK) & 31,604,548       & 583~ns            & 1.83~$\mu$s \\
\texttt{ristretto} (BP-Wrapper)  & 10,557,365       & 292~ns            & 64.83~$\mu$s \\
\texttt{Mutex LRU} (Naive)       & 3,955,855        & 458~ns            & 50.58~$\mu$s \\ \bottomrule
\end{tabular}
\caption{Baseline throughput and latency comparison against production caches. Latency percentiles in this table were sampled at a 1\% rate to preserve realistic throughput levels during measurement. This 1\% sampling interval captures a slightly different distribution and higher median than the fully instrumented microbenchmark in \S 9.3.}
\end{table}

\texttt{liteLRU} demonstrates an $\approx$8$\times$ throughput advantage over the naive Mutex LRU, and an $\approx$3.0$\times$ throughput advantage over the highly optimized \texttt{ristretto} concurrent cache. Furthermore, \texttt{liteLRU} trades a slightly higher median latency (583~ns vs 292~ns) for a much tighter tail (p99 1.83~$\mu$s vs 64.8~$\mu$s). This is an expected and highly competitive tradeoff: \texttt{ristretto} absorbs common-path operations into background buffers for a fast median, but suffers severe tail latency during concurrent buffer flushes. \texttt{liteLRU} instead performs synchronous, bounded writes.
...
To evaluate eviction fidelity under realistic skewed access patterns, we benchmarked \texttt{liteLRU} against \texttt{ristretto} using a Zipfian distribution ($s=1.001$, $N=100{,}000$ working set) across varying cache capacities.

\begin{table}[h]
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Cache Capacity} & \textbf{\texttt{liteLRU} Hit Rate} & \textbf{\texttt{ristretto} Hit Rate} \\ \midrule
25\% (25,000)           & \textbf{78.73\%}                   & 37.48\%                              \\
50\% (50,000)           & \textbf{78.74\%}                   & 44.68\%                              \\
75\% (75,000)           & \textbf{78.73\%}                   & 51.39\%                              \\
95\% (95,000)           & \textbf{78.73\%}                   & 55.37\%                              \\ \bottomrule
\end{tabular}
\caption{Hit rates under Zipfian distribution.}
\end{table}

Surprisingly, \texttt{liteLRU}'s bitmask-CLOCK approximation achieves a strictly superior and perfectly stable $\sim$78.7\% hit rate across all tested capacities, while \texttt{ristretto}'s SampledLFU struggles to warm up and properly admit items under this specific bursty Zipfian workload without a prolonged training period. Furthermore, because \texttt{liteLRU} features a wait-free hit path without the lock-based delinking overhead of traditional LRU, its throughput remains immune to the high hit-ratio contention pathology formally described by Qiu et al.~\cite{hitrate2026}.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Workload Mix} & \textbf{\texttt{liteLRU} Ops/sec} & \textbf{\texttt{ristretto} Ops/sec} & \textbf{Speedup} \\ \midrule
50/50 Get/Add         & \textbf{18,217,269}               & 6,008,203                           & \textbf{3.03x}   \\
20/80 Get/Add         & \textbf{10,820,214}               & 2,850,719                           & \textbf{3.79x}   \\ \bottomrule
\end{tabular}
\caption{Throughput under write-heavy pressure.}
\end{table}

\texttt{liteLRU} demonstrates a 3.0x to 3.7x throughput advantage under write-heavy pressure. Even when writes dominate (80\%), \texttt{liteLRU} sustains $>$10M ops/sec by confining state mutations to single-cycle hardware atomics (the $W_k$ bitmask) rather than coarse-grained shard locks or blocking background channels.

\section{Discussion and Limitations}

\textbf{Memory Overhead.} The tradeoff for 64-byte padded seqlocks and SoA alignment is significantly increased memory overhead per entry compared to dense tag-based implementations like MemC3. A \texttt{liteLRU} entry requires approximately 104 bytes of overhead (64 bytes for the seqlock + 40 bytes for atomic pointers/lengths). For a 1,000,000 entry cache, this requires \textbf{$\sim$133 MB} of heap allocation for the SoA arrays, whereas \texttt{ristretto} requires only \textbf{$\sim$48 MB}. This overhead is acceptable for routing caches holding megabytes of string data, but wasteful for caches scaling into the billions of extremely small elements.
